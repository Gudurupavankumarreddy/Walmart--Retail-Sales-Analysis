...
-- YoY Shows growth/decline by year
WITH yearly AS (
    SELECT 
        Year,
        SUM(Weekly_Sales) AS total_sales
...
SELECT 
    ...
    total_sales - LAG(total_sales) OVER (ORDER BY Year) AS change_dollars,
...
-- Calculates holiday lift percentage
SELECT 
...
-- Shows which months have strongest holiday effect
WITH monthly_sales AS (
    SELECT Month, Holiday_Flag, AVG(Weekly_Sales) AS avg_sales
    FROM walmart_sales
    GROUP BY Month, Holiday_Flag)
SELECT 
    h.Month,
...
-- Identifies high-risk stores (high unemployment + low sales)
...
SELECT Store,
    ...
    CASE WHEN avg_unemployment >= 10 AND total_sales < 15000000 THEN 'Critical Risk'
         WHEN avg_unemployment >= 9 OR total_sales < 10000000 THEN 'High Risk'
         WHEN avg_unemployment >= 7.5 THEN 'Medium Risk'
         ELSE 'Low Risk'
    END AS risk_category
FROM store_metrics
ORDER BY CASE WHEN risk_category = 'Critical Risk' THEN 1
              WHEN risk_category = 'High Risk' THEN 2
		ELSE 3 END, total_sales;

-- Seasonality Index
-- Shows which months are above/below average
...
SELECT 
    ...
    CASE 
        WHEN (m.avg_sales / o.overall_avg) * 100 >= 110 THEN 'Peak'
        WHEN (m.avg_sales / o.overall_avg) * 100 <= 90 THEN 'Slow'
        ELSE 'Normal'
    END AS season
FROM monthly m
